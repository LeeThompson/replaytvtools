...
**
** For additional detail on the Replay 4000 internals, protocols and structures I highly recommend
...
#undef SHOWSTRUCTURESIZES

#define REPLAYSHOW      444             // needed size of replayshow structure
#define REPLAYCHANNEL   624             // needed size of replaychannel structure
#define GUIDEHEADER     808             // needed size of guideheader structure
...
#define lpszTitle		"ReplayTV 4000 GuideParser v1.0 (Build 12)"
...
**
**  The RTV4K platform is capable of supporting NTSC, PAL and SECAM so in 
**  theory there may be one or more bytes always set a certain way as they
**  reflect
**  NTSC.
**
**  On the subject of recorded time: The ReplayTV 4000's appear to 
...
** Categories
**
**  There appears to be an *internal* maximum of 32 but the 4000's appear
**  to only use 17 total.
** 
**  Categories start at byte 404, a maximum of 9 characters and are each null 
**  terminated.  Use GuideHeader.categories to count them off.
**
**
...
**
** They may also be a memory structure at has no meaning on a 'remote unit'.
**
**************************************************************************/

typedef struct tagGuideSnapshotHeader { 
    WORD major;                		// Major Revision (3)
    WORD minor;                		// Minor Revision (1)
    DWORD unknown1;				    // Seems to always be 0x20 (magic)
    DWORD replaychannels;			// Number of Replay Channels
    DWORD replaychannels2;			// Number of Replay Channels (Copy 2; should always match)
...
typedef struct tagThemeInfo {
    ...
    char szSearchString[52];    	// Search Text
} ThemeInfo;
...
typedef struct tagReplayChannel { 
    ...
    DWORD seconds_allocated;   	  	// For guaranteed shows, the total number of seconds allocated 	   
    char szShowLabel[48];	    	// Show Label 
...
void ConvertReplayChannelEndian(struct tagReplayChannel * strReplayChannel)
{
    ...
    strReplayChannel->seconds_allocated = ntohl(strReplayChannel->seconds_allocated);
...
void ConvertReplayGuideEndian(struct tagGuideHeader * strGuideHeader)
{
    ...
    strGuideHeader->GuideSnapshotHeader.major = ntohs(strGuideHeader->GuideSnapshotHeader.major);
    strGuideHeader->GuideSnapshotHeader.minor = ntohs(strGuideHeader->GuideSnapshotHeader.minor);
    strGuideHeader->GuideSnapshotHeader.replaychannels = ntohl(strGuideHeader->GuideSnapshotHeader.replaychannels);
    strGuideHeader->GuideSnapshotHeader.replaychannels2 = ntohl(strGuideHeader->GuideSnapshotHeader.replaychannels2);
...
int main(int argc, char* argv[])
{
    ...
    bool fmodeExtractRGH = false;
    bool fmodeExtractRGS = false;
    bool fmodeExtractRGC = false;
    ...
    while (iarg < argc)
    {
        ...
        else if (!strcmp(argv[iarg], "-eh"))
        {
            fmodeExtractRGH = true;
        }
        else if (!strcmp(argv[iarg], "-es"))
        {
            fmodeExtractRGS = true;
        }
        else if (!strcmp(argv[iarg], "-ec"))
        {
            fmodeExtractRGC = true;
        }
        ...
        {
            fmodeExtractRGC = false;
            fmodeExtractRGS = false;
            fmodeExtractRGH = false;
    ...
    if (!fmodeList)
    {
        if ((!fmodeExtractRGH) && (!fmodeExtractRGS) && (!fmodeExtractRGC)) 
        {
            // If nothing is defined, use legacy syntax
    ...
    if (fmodeShowHelp)
    {
        ...
        printf("    -eh             Extract Header\n");
        printf("    -es             Extract Shows\n");
        printf("    -ec             Extract Channels\n");
        ...
        printf("    guideparser -eh -es -ec -f guide.dat    Parse Guide and Extract to Files\n");
   ...
   if (fmodeDebug)
    ...
    if (strGuideHeader.GuideSnapshotHeader.major != 3) 
    {
        printf("Guide file is damaged! (v %d.%d)\n",strGuideHeader.GuideSnapshotHeader.major,strGuideHeader.GuideSnapshotHeader.minor);
        FILE *fesg;
        fesg = fopen("dump.rgh", "wb");
        if (!fesg)
        {
            printf("ERROR: can't open dump.rgh\n");
        }
        else
        {
            GuideHeader ghout = strGuideHeader;
            
            fwrite(&ghout, sizeof(GuideHeader), sizeof(char), fesg);
            fclose(fesg);
        }
        fclose( guidefile );
        return 0;
    ...
    {
        FILE *fesg;
        fesg = fopen("guide.rgh", "wb");
        if (!fesg)
        {
            printf("ERROR: can't open guide.rgh\n");
        }
        else
        {
            GuideHeader ghout = strGuideHeader;
            
            //				ghout.replaychannels = 0;
            //				ghout.replaychannels2 = 0;
            //				ghout.replaychannels3 = 0;
    ...
    if (fmodeList)
    {
        printf("You have %u ReplayChannels.\n",strGuideHeader.GuideSnapshotHeader.replaychannels);
    }

    if (strGuideHeader.GuideSnapshotHeader.replaychannels > MAXCHANNELS)
    {
    ...
    maxshow = strGuideHeader.GuideSnapshotHeader.replaychannels;		
    ...
    if ((fmodeListChannels) || (fmodeExtractRGC) || (fmodeDetailShowListings))
    {
        ...
        do
        {
            ...
            {
                char szfname[20];
                unsigned int nt = ntohl(strReplayChannel.created);
                sprintf(szfname, "%d.rgc", nt);
                
                FILE *fn = fopen(szfname, "wb");
                if (fn)
                {
                    fwrite(&strReplayChannel, sizeof(tagReplayChannel), sizeof(char), fn);
                    fclose(fn);
                }
            }
        ...
        if ((fmodeListShows) || (fmodeExtractRGS))
        {
            ...
            do
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            char szfname[20];
                            unsigned int nt = ntohl(strReplayShow.recorded);
                            sprintf(szfname, "%d.rgs", nt);
                            
                            FILE *fn = fopen(szfname, "wb");
                            if (fn)
                            {
                                fwrite(&strReplayShow, sizeof(tagReplayShow), sizeof(char), fn);
                                fclose(fn);
                            }
                        }
